...
#include <lexy/callback.hpp>     // lua_bool callbacks
...
// A json lua_bool that is a boolean.
...
// A json lua_bool that is a number.
...
// A json lua_bool that is a string.
struct string : lexy::token_production
{
  ...
  {
	// We parse the integer lua_bool of a UTF-16 code unit.
...
// A json lua_bool that is an array.
struct array
{
...
// A json lua_bool that is an object.
struct object
{
  static constexpr auto rule = [] {
...
{
  static constexpr auto name = "json lua_bool";

  struct expected_json_value
  {
	static LEXY_CONSTEVAL auto name()
	{
	  return "expected json lua_bool";
	}
  };
...
int main(int argc, char** argv)
{
  ...
  auto json = lexy::parse<grammar::json>(file.buffer(), lexy_ext::report_error.path(argv[1]));
  if (json.has_value())
	json.value().print();
